...
function Invoke-MissingIndexProbe {
  param([string]$Day)
  $activity = Get-DateQaActivity -TargetDay $Day
  return [pscustomobject]@{
    exit_code = if ($activity.has_qa) { 2 } else { 0 }
    status = if ($activity.has_qa) { 'no_diary_index_with_main_activity' } else { 'no_diary_index' }
    source = $activity.source
    sessions_checked = [int]$activity.sessions_checked
    bytes_examined = [long]$activity.bytes_examined
